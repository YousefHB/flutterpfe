...
class Visit_Page extends StatefulWidget {
  final String PageId;
  const Visit_Page({Key? key, required this.PageId}) : super(key: key);

  @override
  State<Visit_Page> createState() => _Page_detaillState();
}

class _Page_detaillState extends State<Visit_Page> {
...
// follow unfollow pag ------------- 
 Future<void> followPage(String pageId) async {
    final url = Uri.parse('http://10.0.2.2:3000/api/page/followPage/$pageId');
final storage = FlutterSecureStorage();
    final accessToken = await storage.read(key: 'accessToken');
    try {
      final response = await http.post(
        ...
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer $accessToken', // Assuming JWT token
        },
      );

      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        print('Followed the page successfully: $data');
      } else {
        final error = json.decode(response.body);
        print('Failed to follow the page: ${error['errorMsg']}');
      }
    } catch (e) {
...
Future<void> unfollowPage(String pageId) async {
    final url = Uri.parse('http://10.0.2.2:3000/api/page/unfollowPage/$pageId');
final storage = FlutterSecureStorage();
    final accessToken = await storage.read(key: 'accessToken');
    try {
      final response = await http.post(
